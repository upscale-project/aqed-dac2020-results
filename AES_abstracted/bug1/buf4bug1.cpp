...
//    memcpy(local_key[0], key, 32);
    ...
    major_loop: for (i=0; i<num_batches; i++) {
        ...
        // FIXME: the commented statement is the correct one
	// for(k=0;k<BUF_SIZE/UNROLL_FACTOR;k++){
        //      buf[j][k] = *(data + j*BUF_SIZE/UNROLL_FACTOR + i*BUF_SIZE + k);
	    for(k=0;k<BUF_SIZE;k++){
       //         #pragma HLS UNROLL
        	buf[j][k] = *(data + j*BUF_SIZE + i*BUF_SIZE + k);
	    }
// Converted from:  memcpy(buf[j], data + i*BUF_SIZE + j*BUF_SIZE, BUF_SIZE);
	}
	...
	#pragma HLS UNROLL
        ...
        // FIXME: the commented statement is the correct one
	// for(k=0;k<BUF_SIZE/UNROLL_FACTOR;k++){
        //      *(data + j*BUF_SIZE/UNROLL_FACTOR + i*BUF_SIZE + k) = buf[j][k];
	    for(k=0;k<BUF_SIZE;k++){
       //         #pragma HLS UNROLL
		*(data + i*BUF_SIZE + j*BUF_SIZE + k) = buf[j][k] ;
	    }

// Converted from:   memcpy(data + i*BUF_SIZE + j*BUF_SIZE, buf[j], BUF_SIZE);
	}
    }
    return;
}
...
out1 aqed_in (uint8_t *bmc_in, uint8_t *key, ap_uint<1> orig, ap_uint<1> dup, uint8_t orig_idx, uint8_t dup_idx, uint8_t num) {
...
#pragma design top
out3 aqed_top (ap_uint<1> orig, ap_uint<1> dup, uint8_t orig_idx, uint8_t dup_idx) {
...
uint8_t num = 4;
uint8_t aqed_num = num/AQED_SIZE;

o1 = aqed_in(&bmc_in[idx_ct*BUF_SIZE], key_in, orig, dup, orig_idx, dup_idx, aqed_num);

workload(key_in, &bmc_in[idx_ct*BUF_SIZE], num);

o2 = aqed_out(&bmc_in[idx_ct*BUF_SIZE]);
