// ==============================================================
// RTL generated by Vivado(TM) HLS - High-Level Synthesis from C, C++ and SystemC
// Version: 2018.3
// Copyright (C) 1986-2018 Xilinx, Inc. All Rights Reserved.
// 
// ===========================================================

`timescale 1 ns / 1 ps 

module workload (
        ap_clk,
        ap_rst,
        ap_start,
        ap_done,
        ap_idle,
        ap_ready,
        data_address0,
        data_ce0,
        data_we0,
        data_d0,
        data_q0
);

parameter    ap_ST_fsm_state1 = 11'd1;
parameter    ap_ST_fsm_state2 = 11'd2;
parameter    ap_ST_fsm_state3 = 11'd4;
parameter    ap_ST_fsm_state4 = 11'd8;
parameter    ap_ST_fsm_state5 = 11'd16;
parameter    ap_ST_fsm_state6 = 11'd32;
parameter    ap_ST_fsm_state7 = 11'd64;
parameter    ap_ST_fsm_state8 = 11'd128;
parameter    ap_ST_fsm_state9 = 11'd256;
parameter    ap_ST_fsm_state10 = 11'd512;
parameter    ap_ST_fsm_state11 = 11'd1024;

input   ap_clk;
input   ap_rst;
input   ap_start;
output   ap_done;
output   ap_idle;
output   ap_ready;
output  [3:0] data_address0;
output   data_ce0;
output   data_we0;
output  [7:0] data_d0;
input  [7:0] data_q0;

reg ap_done;
reg ap_idle;
reg ap_ready;
reg[3:0] data_address0;
reg data_ce0;
reg data_we0;

(* fsm_encoding = "none" *) reg   [10:0] ap_CS_fsm;
wire    ap_CS_fsm_state1;
wire   [7:0] local_key_0_q0;
wire   [7:0] local_key_1_q0;
wire   [5:0] j_2_fu_379_p2;
wire    ap_CS_fsm_state2;
wire   [1:0] j_4_fu_391_p2;
reg   [1:0] j_4_reg_560;
wire    ap_CS_fsm_state5;
wire   [0:0] tmp_1_fu_397_p1;
reg   [0:0] tmp_1_reg_565;
wire   [0:0] exitcond3_fu_385_p2;
wire   [2:0] tmp_2_fu_401_p3;
reg   [2:0] tmp_2_reg_569;
wire   [2:0] k_2_fu_415_p2;
reg   [2:0] k_2_reg_577;
wire    ap_CS_fsm_state6;
wire   [0:0] exitcond5_fu_409_p2;
wire   [0:0] tmp_7_fu_431_p1;
reg   [0:0] tmp_7_reg_587;
wire   [7:0] buf_1_1_7_fu_435_p3;
wire    ap_CS_fsm_state7;
wire   [7:0] buf_1_1_8_fu_442_p3;
wire   [7:0] buf_1_1_4_fu_449_p3;
wire   [7:0] buf_1_1_6_fu_456_p3;
reg   [7:0] buf_0_0_3_reg_615;
wire    ap_CS_fsm_state9;
wire    grp_aes256_encrypt_ecb_fu_349_ap_ready;
wire    grp_aes256_encrypt_ecb_fu_349_ap_done;
wire    grp_aes256_encrypt_ecb_fu_361_ap_ready;
wire    grp_aes256_encrypt_ecb_fu_361_ap_done;
reg    ap_block_state9_on_subcall_done;
reg   [7:0] buf_0_1_3_reg_621;
reg   [7:0] buf_1_0_3_reg_627;
reg   [7:0] buf_1_1_3_reg_633;
wire   [1:0] j_5_fu_485_p2;
reg   [1:0] j_5_reg_642;
wire    ap_CS_fsm_state10;
wire   [0:0] tmp_3_fu_491_p1;
reg   [0:0] tmp_3_reg_647;
wire   [0:0] exitcond6_fu_479_p2;
wire   [2:0] tmp_9_fu_495_p3;
reg   [2:0] tmp_9_reg_652;
wire   [2:0] k_3_fu_509_p2;
wire    ap_CS_fsm_state11;
wire    grp_aes256_encrypt_ecb_fu_349_ap_start;
wire    grp_aes256_encrypt_ecb_fu_349_ap_idle;
wire   [4:0] grp_aes256_encrypt_ecb_fu_349_k_address0;
wire    grp_aes256_encrypt_ecb_fu_349_k_ce0;
wire   [7:0] grp_aes256_encrypt_ecb_fu_349_ap_return_0;
wire   [7:0] grp_aes256_encrypt_ecb_fu_349_ap_return_1;
wire    grp_aes256_encrypt_ecb_fu_361_ap_start;
wire    grp_aes256_encrypt_ecb_fu_361_ap_idle;
wire   [4:0] grp_aes256_encrypt_ecb_fu_361_k_address0;
wire    grp_aes256_encrypt_ecb_fu_361_k_ce0;
wire   [7:0] grp_aes256_encrypt_ecb_fu_361_ap_return_0;
wire   [7:0] grp_aes256_encrypt_ecb_fu_361_ap_return_1;
reg   [5:0] j_reg_93;
wire   [0:0] exitcond2_fu_373_p2;
reg   [7:0] buf_1_1_reg_104;
reg   [7:0] buf_1_0_reg_116;
reg   [7:0] buf_0_1_reg_128;
reg   [7:0] buf_0_0_reg_140;
wire   [0:0] ap_phi_mux_i_2_phi_fu_157_p4;
reg   [0:0] i_2_reg_152;
wire    ap_CS_fsm_state3;
reg   [7:0] buf_1_1_1_reg_165;
wire    ap_CS_fsm_state4;
reg   [7:0] buf_1_0_1_reg_176;
reg   [7:0] buf_0_1_1_reg_187;
reg   [7:0] buf_0_0_1_reg_198;
reg   [1:0] j_1_reg_209;
reg   [7:0] buf_1_1_2_reg_220;
wire    ap_CS_fsm_state8;
reg   [7:0] buf_1_0_2_reg_232;
reg   [7:0] buf_0_1_2_reg_244;
reg   [7:0] buf_0_0_2_reg_256;
reg   [2:0] k_reg_268;
reg   [7:0] buf_1_1_2_be_reg_279;
reg   [7:0] buf_1_0_2_be_reg_291;
reg   [7:0] buf_0_1_2_be_reg_303;
reg   [7:0] buf_0_0_2_be_reg_315;
reg   [1:0] j_3_reg_327;
wire   [0:0] exitcond_fu_503_p2;
reg   [2:0] k_1_reg_338;
reg    grp_aes256_encrypt_ecb_fu_349_ap_start_reg;
reg    grp_aes256_encrypt_ecb_fu_361_ap_start_reg;
wire   [63:0] sum_cast_fu_426_p1;
wire   [63:0] sum2_cast_fu_544_p1;
wire   [2:0] tmp7_fu_421_p2;
wire   [0:0] tmp_8_fu_515_p1;
wire   [7:0] buf_1_load_phi_fu_519_p3;
wire   [7:0] buf_0_load_phi_fu_525_p3;
wire   [2:0] tmp8_fu_539_p2;
reg   [10:0] ap_NS_fsm;

// power-on initialization
initial begin
#0 ap_CS_fsm = 11'd1;
#0 grp_aes256_encrypt_ecb_fu_349_ap_start_reg = 1'b0;
#0 grp_aes256_encrypt_ecb_fu_361_ap_start_reg = 1'b0;
end

workload_local_key_0 #(
    .DataWidth( 8 ),
    .AddressRange( 32 ),
    .AddressWidth( 5 ))
local_key_0_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .address0(grp_aes256_encrypt_ecb_fu_349_k_address0),
    .ce0(grp_aes256_encrypt_ecb_fu_349_k_ce0),
    .q0(local_key_0_q0)
);

workload_local_key_0 #(
    .DataWidth( 8 ),
    .AddressRange( 32 ),
    .AddressWidth( 5 ))
local_key_1_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .address0(grp_aes256_encrypt_ecb_fu_361_k_address0),
    .ce0(grp_aes256_encrypt_ecb_fu_361_k_ce0),
    .q0(local_key_1_q0)
);

aes256_encrypt_ecb grp_aes256_encrypt_ecb_fu_349(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(grp_aes256_encrypt_ecb_fu_349_ap_start),
    .ap_done(grp_aes256_encrypt_ecb_fu_349_ap_done),
    .ap_idle(grp_aes256_encrypt_ecb_fu_349_ap_idle),
    .ap_ready(grp_aes256_encrypt_ecb_fu_349_ap_ready),
    .k_address0(grp_aes256_encrypt_ecb_fu_349_k_address0),
    .k_ce0(grp_aes256_encrypt_ecb_fu_349_k_ce0),
    .k_q0(local_key_0_q0),
    .p_read(buf_0_0_1_reg_198),
    .p_read1(buf_0_1_1_reg_187),
    .ap_return_0(grp_aes256_encrypt_ecb_fu_349_ap_return_0),
    .ap_return_1(grp_aes256_encrypt_ecb_fu_349_ap_return_1)
);

aes256_encrypt_ecb grp_aes256_encrypt_ecb_fu_361(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(grp_aes256_encrypt_ecb_fu_361_ap_start),
    .ap_done(grp_aes256_encrypt_ecb_fu_361_ap_done),
    .ap_idle(grp_aes256_encrypt_ecb_fu_361_ap_idle),
    .ap_ready(grp_aes256_encrypt_ecb_fu_361_ap_ready),
    .k_address0(grp_aes256_encrypt_ecb_fu_361_k_address0),
    .k_ce0(grp_aes256_encrypt_ecb_fu_361_k_ce0),
    .k_q0(local_key_1_q0),
    .p_read(buf_1_0_1_reg_176),
    .p_read1(buf_1_1_1_reg_165),
    .ap_return_0(grp_aes256_encrypt_ecb_fu_361_ap_return_0),
    .ap_return_1(grp_aes256_encrypt_ecb_fu_361_ap_return_1)
);

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_CS_fsm <= ap_ST_fsm_state1;
    end else begin
        ap_CS_fsm <= ap_NS_fsm;
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        grp_aes256_encrypt_ecb_fu_349_ap_start_reg <= 1'b0;
    end else begin
        if (((exitcond3_fu_385_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state5))) begin
            grp_aes256_encrypt_ecb_fu_349_ap_start_reg <= 1'b1;
        end else if ((grp_aes256_encrypt_ecb_fu_349_ap_ready == 1'b1)) begin
            grp_aes256_encrypt_ecb_fu_349_ap_start_reg <= 1'b0;
        end
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        grp_aes256_encrypt_ecb_fu_361_ap_start_reg <= 1'b0;
    end else begin
        if (((exitcond3_fu_385_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state5))) begin
            grp_aes256_encrypt_ecb_fu_361_ap_start_reg <= 1'b1;
        end else if ((grp_aes256_encrypt_ecb_fu_361_ap_ready == 1'b1)) begin
            grp_aes256_encrypt_ecb_fu_361_ap_start_reg <= 1'b0;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((exitcond5_fu_409_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state6))) begin
        buf_0_0_1_reg_198 <= buf_0_0_2_reg_256;
    end else if (((ap_phi_mux_i_2_phi_fu_157_p4 == 1'd0) & (1'b1 == ap_CS_fsm_state4))) begin
        buf_0_0_1_reg_198 <= buf_0_0_reg_140;
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state7)) begin
        if ((tmp_1_reg_565 == 1'd0)) begin
            buf_0_0_2_be_reg_315 <= buf_1_1_8_fu_442_p3;
        end else if ((tmp_1_reg_565 == 1'd1)) begin
            buf_0_0_2_be_reg_315 <= buf_0_0_2_reg_256;
        end
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state8)) begin
        buf_0_0_2_reg_256 <= buf_0_0_2_be_reg_315;
    end else if (((exitcond3_fu_385_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state5))) begin
        buf_0_0_2_reg_256 <= buf_0_0_1_reg_198;
    end
end

always @ (posedge ap_clk) begin
    if (((exitcond5_fu_409_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state6))) begin
        buf_0_1_1_reg_187 <= buf_0_1_2_reg_244;
    end else if (((ap_phi_mux_i_2_phi_fu_157_p4 == 1'd0) & (1'b1 == ap_CS_fsm_state4))) begin
        buf_0_1_1_reg_187 <= buf_0_1_reg_128;
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state7)) begin
        if ((tmp_1_reg_565 == 1'd0)) begin
            buf_0_1_2_be_reg_303 <= buf_1_1_7_fu_435_p3;
        end else if ((tmp_1_reg_565 == 1'd1)) begin
            buf_0_1_2_be_reg_303 <= buf_0_1_2_reg_244;
        end
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state8)) begin
        buf_0_1_2_reg_244 <= buf_0_1_2_be_reg_303;
    end else if (((exitcond3_fu_385_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state5))) begin
        buf_0_1_2_reg_244 <= buf_0_1_1_reg_187;
    end
end

always @ (posedge ap_clk) begin
    if (((exitcond5_fu_409_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state6))) begin
        buf_1_0_1_reg_176 <= buf_1_0_2_reg_232;
    end else if (((ap_phi_mux_i_2_phi_fu_157_p4 == 1'd0) & (1'b1 == ap_CS_fsm_state4))) begin
        buf_1_0_1_reg_176 <= buf_1_0_reg_116;
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state7)) begin
        if ((tmp_1_reg_565 == 1'd0)) begin
            buf_1_0_2_be_reg_291 <= buf_1_0_2_reg_232;
        end else if ((tmp_1_reg_565 == 1'd1)) begin
            buf_1_0_2_be_reg_291 <= buf_1_1_6_fu_456_p3;
        end
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state8)) begin
        buf_1_0_2_reg_232 <= buf_1_0_2_be_reg_291;
    end else if (((exitcond3_fu_385_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state5))) begin
        buf_1_0_2_reg_232 <= buf_1_0_1_reg_176;
    end
end

always @ (posedge ap_clk) begin
    if (((exitcond5_fu_409_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state6))) begin
        buf_1_1_1_reg_165 <= buf_1_1_2_reg_220;
    end else if (((ap_phi_mux_i_2_phi_fu_157_p4 == 1'd0) & (1'b1 == ap_CS_fsm_state4))) begin
        buf_1_1_1_reg_165 <= buf_1_1_reg_104;
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state7)) begin
        if ((tmp_1_reg_565 == 1'd0)) begin
            buf_1_1_2_be_reg_279 <= buf_1_1_2_reg_220;
        end else if ((tmp_1_reg_565 == 1'd1)) begin
            buf_1_1_2_be_reg_279 <= buf_1_1_4_fu_449_p3;
        end
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state8)) begin
        buf_1_1_2_reg_220 <= buf_1_1_2_be_reg_279;
    end else if (((exitcond3_fu_385_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state5))) begin
        buf_1_1_2_reg_220 <= buf_1_1_1_reg_165;
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state3)) begin
        i_2_reg_152 <= 1'd0;
    end else if (((1'b1 == ap_CS_fsm_state10) & (exitcond6_fu_479_p2 == 1'd1))) begin
        i_2_reg_152 <= 1'd1;
    end
end

always @ (posedge ap_clk) begin
    if (((exitcond5_fu_409_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state6))) begin
        j_1_reg_209 <= j_4_reg_560;
    end else if (((ap_phi_mux_i_2_phi_fu_157_p4 == 1'd0) & (1'b1 == ap_CS_fsm_state4))) begin
        j_1_reg_209 <= 2'd0;
    end
end

always @ (posedge ap_clk) begin
    if (((1'b1 == ap_CS_fsm_state9) & (1'b0 == ap_block_state9_on_subcall_done))) begin
        j_3_reg_327 <= 2'd0;
    end else if (((1'b1 == ap_CS_fsm_state11) & (exitcond_fu_503_p2 == 1'd1))) begin
        j_3_reg_327 <= j_5_reg_642;
    end
end

always @ (posedge ap_clk) begin
    if (((1'b1 == ap_CS_fsm_state1) & (ap_start == 1'b1))) begin
        j_reg_93 <= 6'd0;
    end else if (((exitcond2_fu_373_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state2))) begin
        j_reg_93 <= j_2_fu_379_p2;
    end
end

always @ (posedge ap_clk) begin
    if (((exitcond_fu_503_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state11))) begin
        k_1_reg_338 <= k_3_fu_509_p2;
    end else if (((exitcond6_fu_479_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state10))) begin
        k_1_reg_338 <= 3'd0;
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state8)) begin
        k_reg_268 <= k_2_reg_577;
    end else if (((exitcond3_fu_385_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state5))) begin
        k_reg_268 <= 3'd0;
    end
end

always @ (posedge ap_clk) begin
    if (((1'b1 == ap_CS_fsm_state9) & (1'b0 == ap_block_state9_on_subcall_done))) begin
        buf_0_0_3_reg_615 <= grp_aes256_encrypt_ecb_fu_349_ap_return_0;
        buf_0_1_3_reg_621 <= grp_aes256_encrypt_ecb_fu_349_ap_return_1;
        buf_1_0_3_reg_627 <= grp_aes256_encrypt_ecb_fu_361_ap_return_0;
        buf_1_1_3_reg_633 <= grp_aes256_encrypt_ecb_fu_361_ap_return_1;
    end
end

always @ (posedge ap_clk) begin
    if (((1'b1 == ap_CS_fsm_state10) & (exitcond6_fu_479_p2 == 1'd1))) begin
        buf_0_0_reg_140 <= buf_0_0_3_reg_615;
        buf_0_1_reg_128 <= buf_0_1_3_reg_621;
        buf_1_0_reg_116 <= buf_1_0_3_reg_627;
        buf_1_1_reg_104 <= buf_1_1_3_reg_633;
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state5)) begin
        j_4_reg_560 <= j_4_fu_391_p2;
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state10)) begin
        j_5_reg_642 <= j_5_fu_485_p2;
    end
end

always @ (posedge ap_clk) begin
    if ((1'b1 == ap_CS_fsm_state6)) begin
        k_2_reg_577 <= k_2_fu_415_p2;
    end
end

always @ (posedge ap_clk) begin
    if (((exitcond3_fu_385_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state5))) begin
        tmp_1_reg_565 <= tmp_1_fu_397_p1;
        tmp_2_reg_569[2] <= tmp_2_fu_401_p3[2];
    end
end

always @ (posedge ap_clk) begin
    if (((exitcond6_fu_479_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state10))) begin
        tmp_3_reg_647 <= tmp_3_fu_491_p1;
        tmp_9_reg_652[2] <= tmp_9_fu_495_p3[2];
    end
end

always @ (posedge ap_clk) begin
    if (((exitcond5_fu_409_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state6))) begin
        tmp_7_reg_587 <= tmp_7_fu_431_p1;
    end
end

always @ (*) begin
    if ((((1'b1 == ap_CS_fsm_state4) & (ap_phi_mux_i_2_phi_fu_157_p4 == 1'd1)) | ((ap_start == 1'b0) & (1'b1 == ap_CS_fsm_state1)))) begin
        ap_done = 1'b1;
    end else begin
        ap_done = 1'b0;
    end
end

always @ (*) begin
    if (((ap_start == 1'b0) & (1'b1 == ap_CS_fsm_state1))) begin
        ap_idle = 1'b1;
    end else begin
        ap_idle = 1'b0;
    end
end

always @ (*) begin
    if (((1'b1 == ap_CS_fsm_state4) & (ap_phi_mux_i_2_phi_fu_157_p4 == 1'd1))) begin
        ap_ready = 1'b1;
    end else begin
        ap_ready = 1'b0;
    end
end

always @ (*) begin
    if ((1'b1 == ap_CS_fsm_state11)) begin
        data_address0 = sum2_cast_fu_544_p1;
    end else if ((1'b1 == ap_CS_fsm_state6)) begin
        data_address0 = sum_cast_fu_426_p1;
    end else begin
        data_address0 = 'bx;
    end
end

always @ (*) begin
    if (((1'b1 == ap_CS_fsm_state6) | (1'b1 == ap_CS_fsm_state11))) begin
        data_ce0 = 1'b1;
    end else begin
        data_ce0 = 1'b0;
    end
end

always @ (*) begin
    if (((exitcond_fu_503_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state11))) begin
        data_we0 = 1'b1;
    end else begin
        data_we0 = 1'b0;
    end
end

always @ (*) begin
    case (ap_CS_fsm)
        ap_ST_fsm_state1 : begin
            if (((1'b1 == ap_CS_fsm_state1) & (ap_start == 1'b1))) begin
                ap_NS_fsm = ap_ST_fsm_state2;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state1;
            end
        end
        ap_ST_fsm_state2 : begin
            if (((exitcond2_fu_373_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state2))) begin
                ap_NS_fsm = ap_ST_fsm_state2;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state3;
            end
        end
        ap_ST_fsm_state3 : begin
            ap_NS_fsm = ap_ST_fsm_state4;
        end
        ap_ST_fsm_state4 : begin
            if (((1'b1 == ap_CS_fsm_state4) & (ap_phi_mux_i_2_phi_fu_157_p4 == 1'd1))) begin
                ap_NS_fsm = ap_ST_fsm_state1;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state5;
            end
        end
        ap_ST_fsm_state5 : begin
            if (((exitcond3_fu_385_p2 == 1'd0) & (1'b1 == ap_CS_fsm_state5))) begin
                ap_NS_fsm = ap_ST_fsm_state6;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state9;
            end
        end
        ap_ST_fsm_state6 : begin
            if (((exitcond5_fu_409_p2 == 1'd1) & (1'b1 == ap_CS_fsm_state6))) begin
                ap_NS_fsm = ap_ST_fsm_state5;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state7;
            end
        end
        ap_ST_fsm_state7 : begin
            ap_NS_fsm = ap_ST_fsm_state8;
        end
        ap_ST_fsm_state8 : begin
            ap_NS_fsm = ap_ST_fsm_state6;
        end
        ap_ST_fsm_state9 : begin
            if (((1'b1 == ap_CS_fsm_state9) & (1'b0 == ap_block_state9_on_subcall_done))) begin
                ap_NS_fsm = ap_ST_fsm_state10;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state9;
            end
        end
        ap_ST_fsm_state10 : begin
            if (((1'b1 == ap_CS_fsm_state10) & (exitcond6_fu_479_p2 == 1'd1))) begin
                ap_NS_fsm = ap_ST_fsm_state4;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state11;
            end
        end
        ap_ST_fsm_state11 : begin
            if (((1'b1 == ap_CS_fsm_state11) & (exitcond_fu_503_p2 == 1'd1))) begin
                ap_NS_fsm = ap_ST_fsm_state10;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state11;
            end
        end
        default : begin
            ap_NS_fsm = 'bx;
        end
    endcase
end

assign ap_CS_fsm_state1 = ap_CS_fsm[32'd0];

assign ap_CS_fsm_state10 = ap_CS_fsm[32'd9];

assign ap_CS_fsm_state11 = ap_CS_fsm[32'd10];

assign ap_CS_fsm_state2 = ap_CS_fsm[32'd1];

assign ap_CS_fsm_state3 = ap_CS_fsm[32'd2];

assign ap_CS_fsm_state4 = ap_CS_fsm[32'd3];

assign ap_CS_fsm_state5 = ap_CS_fsm[32'd4];

assign ap_CS_fsm_state6 = ap_CS_fsm[32'd5];

assign ap_CS_fsm_state7 = ap_CS_fsm[32'd6];

assign ap_CS_fsm_state8 = ap_CS_fsm[32'd7];

assign ap_CS_fsm_state9 = ap_CS_fsm[32'd8];

always @ (*) begin
    ap_block_state9_on_subcall_done = ((grp_aes256_encrypt_ecb_fu_361_ap_done == 1'b0) | (grp_aes256_encrypt_ecb_fu_349_ap_done == 1'b0));
end

assign ap_phi_mux_i_2_phi_fu_157_p4 = i_2_reg_152;

assign buf_0_load_phi_fu_525_p3 = ((tmp_8_fu_515_p1[0:0] === 1'b1) ? buf_0_1_3_reg_621 : buf_0_0_3_reg_615);

assign buf_1_1_4_fu_449_p3 = ((tmp_7_reg_587[0:0] === 1'b1) ? data_q0 : buf_1_1_2_reg_220);

assign buf_1_1_6_fu_456_p3 = ((tmp_7_reg_587[0:0] === 1'b1) ? buf_1_0_2_reg_232 : data_q0);

assign buf_1_1_7_fu_435_p3 = ((tmp_7_reg_587[0:0] === 1'b1) ? data_q0 : buf_0_1_2_reg_244);

assign buf_1_1_8_fu_442_p3 = ((tmp_7_reg_587[0:0] === 1'b1) ? buf_0_0_2_reg_256 : data_q0);

assign buf_1_load_phi_fu_519_p3 = ((tmp_8_fu_515_p1[0:0] === 1'b1) ? buf_1_1_3_reg_633 : buf_1_0_3_reg_627);

assign data_d0 = ((tmp_3_reg_647[0:0] === 1'b1) ? buf_1_load_phi_fu_519_p3 : buf_0_load_phi_fu_525_p3);

assign exitcond2_fu_373_p2 = ((j_reg_93 == 6'd32) ? 1'b1 : 1'b0);

assign exitcond3_fu_385_p2 = ((j_1_reg_209 == 2'd2) ? 1'b1 : 1'b0);

assign exitcond5_fu_409_p2 = ((k_reg_268 == 3'd4) ? 1'b1 : 1'b0);

assign exitcond6_fu_479_p2 = ((j_3_reg_327 == 2'd2) ? 1'b1 : 1'b0);

assign exitcond_fu_503_p2 = ((k_1_reg_338 == 3'd4) ? 1'b1 : 1'b0);

assign grp_aes256_encrypt_ecb_fu_349_ap_start = grp_aes256_encrypt_ecb_fu_349_ap_start_reg;

assign grp_aes256_encrypt_ecb_fu_361_ap_start = grp_aes256_encrypt_ecb_fu_361_ap_start_reg;

assign j_2_fu_379_p2 = (j_reg_93 + 6'd1);

assign j_4_fu_391_p2 = (j_1_reg_209 + 2'd1);

assign j_5_fu_485_p2 = (j_3_reg_327 + 2'd1);

assign k_2_fu_415_p2 = (k_reg_268 + 3'd1);

assign k_3_fu_509_p2 = (k_1_reg_338 + 3'd1);

assign sum2_cast_fu_544_p1 = tmp8_fu_539_p2;

assign sum_cast_fu_426_p1 = tmp7_fu_421_p2;

assign tmp7_fu_421_p2 = (tmp_2_reg_569 + k_reg_268);

assign tmp8_fu_539_p2 = (tmp_9_reg_652 + k_1_reg_338);

assign tmp_1_fu_397_p1 = j_1_reg_209[0:0];

assign tmp_2_fu_401_p3 = {{tmp_1_fu_397_p1}, {2'd0}};

assign tmp_3_fu_491_p1 = j_3_reg_327[0:0];

assign tmp_7_fu_431_p1 = k_reg_268[0:0];

assign tmp_8_fu_515_p1 = k_1_reg_338[0:0];

assign tmp_9_fu_495_p3 = {{tmp_3_fu_491_p1}, {2'd0}};

always @ (posedge ap_clk) begin
    tmp_2_reg_569[1:0] <= 2'b00;
    tmp_9_reg_652[1:0] <= 2'b00;
end

endmodule //workload
