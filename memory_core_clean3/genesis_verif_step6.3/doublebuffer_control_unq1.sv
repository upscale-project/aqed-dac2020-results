...
assign autoswitch = ~arbitrary_addr & (write_done | write_done_d1) & (read_done_thresh | read_done | init_state) & (wen | ~rate_matched);//saranyu
...
assign valid = last_line_gate & (rate_matched? valid_from_read : valid_int);
...
always @ (posedge clk or posedge reset) begin
  if(reset) begin
	...
	valid_int <= (valid_from_read | autoswitch) & ~read_done;
	end
    end
  end
end
