...
assign autoswitch = ~arbitrary_addr & (write_done | write_done_d1) & (read_done_thresh | read_done | init_state) & (wen | ~rate_matched);
...
assign valid = last_line_gate & (rate_matched? valid_from_read : valid_int & ~read_done_thresh);
...
always @(*) begin
  ...
  doublebuffer_cen_mem[0] = wen | switch | autoswitch | (read_mux);//saranyu
  doublebuffer_cen_mem[1] = wen | switch | autoswitch | (read_mux);//saranyu
